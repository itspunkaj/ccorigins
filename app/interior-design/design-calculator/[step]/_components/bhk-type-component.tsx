    ...
    return (
        <StepComponent>
            {[1, 2, 3, 4, 5].map((item, index) => {
                ...
                return (
                    <div key={index} onClick={()=>setBHK(item)} className={`cursor-pointer border rounded-md w-full p-4 gap-4 flex items-center ${isSelected && 'bg-yellow-50 border-primaryYellow'}`}>
                        <input
                            id={`bhk-${item}`}
                            checked={isSelected} 
                            name="bhk"
