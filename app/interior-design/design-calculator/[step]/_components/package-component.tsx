    ...
    <StepComponent>
      {packages.map((pkg) => {
        ...
        return (
          <div
            key={pkg.name}
            onClick={() => setPackage(pkg.name)}
            className={`cursor-pointer border rounded-md w-full p-4 gap-4 flex items-start ${isSelected && 'bg-yellow-50 border-primaryYellow'}`}
          >
            <input
              id={`${pkg.name}`}
              checked={isSelected}
            ...
            <label htmlFor={`${pkg.name}`}>
              ...
              <ul>
                {pkg.features.map((feature) => (
                  <li key={feature} className="text-gray-600 font-thin text-sm list-disc">
                    {feature}
                  </li>
                ))}
              </ul>
